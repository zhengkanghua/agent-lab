...
from news_vector_service.api.auth import router as auth_router
from news_vector_service.api.health import router as health_router
from news_vector_service.api.document_search import router as document_search_router
from news_vector_service.api.documents import router as documents_router
from news_vector_service.api.pipeline import router as pipeline_router
from news_vector_service.api.vector_search import (
    VectorSearchRuntimeUnavailableError,
    build_vector_search_error_response,
    router as vector_search_router,
)
from news_vector_service.api.user_admin import router as user_admin_router
from news_vector_service.auth.dependencies import current_active_user, current_superuser
from news_vector_service.auth.bootstrap import (
...
from news_vector_service.config.freshrss import get_freshrss_settings
...
from news_vector_service.pipeline.write_runtime import PipelineWriteRuntime
...
def create_app(
    *,
    ...
    """创建 News Vector Service 的 FastAPI 应用（ASGI 应用）。
    ...
    application = FastAPI(
        title="News Vector Service",
        description=(
            "FreshRSS 新闻增量同步、Ollama/LangChain Embedding、Qdrant 索引与只读"
            "向量搜索服务。流水线接口是显式手动写操作，不会在启动时自动执行。"
        ),
        version="0.1.0",
...
# Uvicorn 通过 ``news_vector_service.main:app`` 导入这个 ASGI 应用对象。
app = create_app()
