...
平台当前范围是只读语义检索。**不提前创建无真实行为的 Chat、RAG、Prompt 或流式模块**
（见平台根 [`AGENTS.md`](../AGENTS.md)）。没有生成式 LLM 调用、没有对话历史、没有
Prompt 模板层、没有 SSE/WebSocket 流式输出。检索结果直接返回原始 Chunk 与 Cosine
score，不做摘要、改写或答案生成。

空接口和占位工厂比缺失的功能更难删除：一个没有第二种实现的 Protocol 会让每个读代码的人
以为存在多态，一个只有一条分支的策略选择器会让调用方以为可以替换。因此只有出现第二种
真实实现时才引入抽象。
...
不做新闻时间加权。前端也不对有限的 Chunk top-k 做伪分组或重排后端已排好的顺序——两边
各算一遍必然漂移。应用不重复做 L2 normalization（Cosine Collection 在上传时已执行）。
...
``POST /vector-search`` 是兼容的 Chunk 级契约，允许同一 document 的多个 Chunk 分别出现；
不修改或删除它的既有语义。错误 ``code`` 只能新增不能改值，包括那两处刻意保留的分叉。
