...
前端对检索结果做任何 `filter` / `sort` / `uniq` 都视为缺陷——两边各算一遍必然漂移。
约束本身是根 `AGENTS.md` 第 6 条，本文记录保证点在哪里、以及为什么前端那一遍必须删掉。
...
chunk 模式下的多 Chunk 断言不要动，那是 `AGENTS.md` 第 7 条允许的正当契约：
`POST /vector-search` 是 Chunk 级契约，同一 document 的多个 Chunk 分别出现是合法的。
