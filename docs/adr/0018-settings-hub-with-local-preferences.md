...
2. **~~偏好只持久化在本浏览器的 localStorage~~**（键 `signaldesk.preferences.v1`，带版本号），
   ...
   **本条已由 [ADR 0027](0027-user-level-preferences-in-database.md) 取代**：三个偏好均迁入
   `user_preferences` 表，localStorage 不再作为事实源。取代的触发条件是提示词的**消费方从客户端
   转移到了服务端**（会话建立时后端要读它），本条原文「消费方本来就由前端组装」的前提随之失效。
...
**这条已被 [ADR 0027](0027-user-level-preferences-in-database.md) 采纳**——当时预判的触发条件
（消费方从客户端转移到后端）真实发生了，原文保留在此以记录当时的权衡。
