...
2026-09-11，老板确认将现有定时任务重构为各业务领域共用的任务组件，采用 Celery Beat、Celery Worker 和 Redis，PostgreSQL 保存受理与恢复依据。2026-09-12 已开始实施，公共组件、业务入口、任务管理及部署配套已进入本地代码；真实 PostgreSQL 隔离迁移、并发、事务交接和三存储故障恢复验收已通过，开发库已按授权升级并核对用户数据保持一致。真实 Linux 队列验收及运行服务切换尚未完成。精确实现与工程进度分别见代码和施工 spec；本文末尾保留重构前的历史决策。

## 已确认的重构方向（2026-09-11，实施中）
...
实施顺序为公共契约和持久受理、Beat／Redis／Worker 接入、现有业务入口迁移、任务管理界面及部署文档联动。迁移执行历史与配置关联，补齐受理快照和必要状态；不能直接删除现有业务待办或依赖旧快照猜测执行结果。即使开发阶段当前没有活动任务，也应验证迁移前提，再停用旧 APScheduler 和消费者，避免新旧执行入口同时处理同一业务。2026-09-12 老板已授权直接修改开发数据库和处理非用户数据，开发库升级已完成；真实服务停止与启动仍须落实对应环境授权。
...
这些是实施验收要求。需求定稿时仅做只读核对；2026-09-12 已完成本地实现与离线回归，执行真实 PostgreSQL／Qdrant／S3 隔离验收，并将开发库升级至 `b6e2f9047a31`，保留账号、凭据、Agent 会话、审核记录及任务身份。实际环境、结果及尚未执行项记录在施工 spec。没有执行生产切换或真实 Redis／Beat／Worker 进程故障实验。
