...
`agent-lab prune-orphan-threads` 清理，默认只预演、加 `--yes` 才真删。删账号时业务表这一行由
`UserAdminService.delete_user` 在同一事务里删掉（库里没有外键约束了，见
[ADR 0028](0028-drop-database-foreign-keys.md)），但 checkpointer 里的历史不在业务表里，
同样靠这个命令回收。
