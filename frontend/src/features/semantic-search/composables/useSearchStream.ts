 ...
 *  - 输入草稿 draft 与全局数量参数 documentLimit / matchesPerDocument：供下一条搜索用
 *    （Q7 甲：参数全局一份，影响之后所有检索轮）。
 *
...
export function useSearchStream() {
  ...
  async function search(): Promise<void> {
    ...
    const limit = normalizeResultLimit(documentLimit.value)
    const perDocument = normalizeMatchesPerDocument(matchesPerDocument.value)
    documentLimit.value = limit
    matchesPerDocument.value = perDocument
