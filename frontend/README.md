...
后台的唯一入口是前台顶栏一枚仅超级用户可见的图标（指向 `/admin`），
三个前台页面共用同一个外壳因此都能到达；设置中心的侧栏只放三个设置分区，
不再挂后台直达链接（原「账号管理」「定时任务」两条已撤，见
`docs/adr/0025-admin-entry-is-a-single-topbar-link.md`）。后台五个分区由 `/admin` 自己的
侧栏承载。
